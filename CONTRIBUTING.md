...
2) Выберите один наиболее подходящий из трёх шаблонов для `Issue` и нажмите `Get Started`.
    ...
    2) При создании `Issue` без шаблона она будет закрыта и пересоздана владельцами репозитория, а сылка на новую `Issue` будет помещена в старой. (_Наказания за это нарушение не предусматривается._)
