...
Установите `NuGet-пакет` [`ABCNET.NETMouseProjects`](https://www.nuget.org/packages/NETMouseProjectsOrganization.ABCNET/0.0.1) через `Подключённые сборки`->`Добавить сборку`->`NuGet`.
...
Установите `NuGet-пакет` [`ABCNET.NETMouseProjects`](https://www.nuget.org/packages/NETMouseProjectsOrganization.ABCNET/0.0.1) через `<project>`->`Manage NuGet Packages`->`Browse`.
